...
function initializeEventListeners() {
  ...
  // Equipment description management
  initializeDescriptionManagement();
...
// Equipment description management
function initializeDescriptionManagement() {
  document.addEventListener('click', async function (e) {
    const addBtn = e.target.closest('#addDescriptionBtn');
    if (addBtn && !isAddingDescription) {
      e.preventDefault();
...
async function handleAddDescription() {
  ...
  const successMessage = document.getElementById('descriptionSuccessMessage');
  const addButton = document.getElementById('addDescriptionBtn');
  const descriptionSelect = document.getElementById('equipment_description_select');

  if (!input || !successMessage || !addButton || !descriptionSelect) {
    console.error('Missing elements for description management');
    showNotification('System configuration error. Please refresh the page.', 'error');
    return;
  }

  const descriptionName = input.value.trim();

  if (!descriptionName) {
    showNotification('Please enter an equipment description name', 'warning');
    input.focus();
    return;
  }

  if (!djangoData.urls?.createDescription) {
    console.error('Create description URL not found');
  ...
  addButton.innerHTML = '<i class="bx bx-loader-alt bx-spin"></i> Adding...';
  ...
  try {
    ...
    const response = await fetch(djangoData.urls.createDescription, {
      method: 'POST',
      headers: {
    ...
    const newOption = new Option(data.description_name, data.description_id);
    descriptionSelect.add(newOption);
    descriptionSelect.value = data.description_id;
    ...
    console.error('Error adding equipment description:', error);
