...
_KENYAN_FORMATTER = KenyanTimeFormatter(
    "%(asctime)s [%(levelname)s] %(name)s - %(message)s", datefmt="%Y-%m-%d %H:%M:%S EAT"
)
...
def setup_logging(log_file: str = None, level: int = logging.DEBUG) -> None:
    ...
    if log_file:
        try:
            ...
            # Bounded (10MB x 5 backups, matching the rest of the app's logs) —
            # a plain FileHandler here previously grew unbounded (one instance
            # reached 55MB) since nothing else in this path capped its size.
    ...
    return {
        ...
        "sync": {
            ...
            "download_interval_seconds": int(
                os.getenv("SYNC_DOWNLOAD_INTERVAL", "1")
            ),  # ⚡ instant polling
        ...
        "client": {"name": os.getenv("CLIENT_NAME", "Workshop")},
        "mirror": {
