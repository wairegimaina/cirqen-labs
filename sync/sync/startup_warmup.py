...
class StartupStateManager:
    ...
    def clear_stale_states(self):
        ...
        # 8. Clean up PostgreSQL stale files
        self._cleanup_postgresql()
    ...
    def _cleanup_postgresql(self):
        """Clean up stale PostgreSQL files and processes"""
        pg_data = self.data_path / 'postgres'

        if not pg_data.exists():
            return

        self.logger.info("🐘 Checking PostgreSQL state...")

        cleaned_items = []

        # 1. Check and clean postmaster.pid
        ...
        if postmaster_pid.exists():
            try:
                ...
                self.logger.info(f"  ⚠️  Found stale postmaster.pid (PID: {old_pid})")
                ...
                if psutil.pid_exists(old_pid):
                    try:
                        ...
                        if 'postgres' in proc.name().lower():
                            self.logger.info(f"  🔪 Terminating stale PostgreSQL process...")
                            proc.terminate()
                            ...
                            try:
                                ...
                                self.logger.warning(f"  ⚠️  Forcing kill...")
                                proc.kill()
                                proc.wait(timeout=5)
                                self.logger.info(f"  ✅ Process killed")

                            # Wait for cleanup
                            time.sleep(2)
                            cleaned_items.append(f"PostgreSQL process (PID: {old_pid})")
                        else:
                            self.logger.info(f"  ℹ️  PID {old_pid} is not PostgreSQL")
                    except psutil.NoSuchProcess:
                        self.logger.info(f"  ℹ️  Process {old_pid} already dead")
                    except Exception as e:
                        self.logger.warning(f"  ⚠️  Error checking process: {e}")
                else:
                    self.logger.info(f"  ℹ️  Process {old_pid} doesn't exist")

                # Remove PID file
                postmaster_pid.unlink()
                self.logger.info(f"  ✅ Removed postmaster.pid")
                cleaned_items.append("PostgreSQL PID file")

            except Exception as e:
                self.warnings.append(f"Could not clean postmaster.pid: {e}")

        # 2. Clean up socket files in data directory
        socket_files = list(pg_data.glob('.s.PGSQL.*'))
        if socket_files:
            self.logger.info(f"  🧹 Cleaning {len(socket_files)} socket file(s)...")
            for socket_file in socket_files:
                try:
                    socket_file.unlink()
                    self.logger.info(f"    ✅ Removed {socket_file.name}")
                except Exception as e:
                    self.warnings.append(f"Could not remove socket {socket_file.name}: {e}")
            cleaned_items.append(f"{len(socket_files)} PostgreSQL socket file(s)")

        # 3. Clean up socket files in /tmp (Unix only)
        if os.name != 'nt':
            try:
                tmp_path = Path('/tmp')
                tmp_sockets = list(tmp_path.glob('.s.PGSQL.2215*'))
                if tmp_sockets:
                    self.logger.info(f"  🧹 Cleaning {len(tmp_sockets)} /tmp socket file(s)...")
                    for socket_file in tmp_sockets:
                        try:
                            socket_file.unlink()
                            self.logger.info(f"    ✅ Removed {socket_file.name}")
                        except Exception as e:
                            self.warnings.append(f"Could not remove /tmp socket: {e}")
                    cleaned_items.append(f"{len(tmp_sockets)} /tmp socket file(s)")
            except Exception as e:
                self.logger.debug(f"Could not check /tmp: {e}")

        # 4. Wait for filesystem sync
        if cleaned_items:
            time.sleep(1)
            self.logger.info(f"  ✅ PostgreSQL cleanup complete")
            self.cleared_items.append(f"PostgreSQL cleanup: {', '.join(cleaned_items)}")
        else:
            self.logger.info(f"  ✅ PostgreSQL state clean")
