...
class RedisQueueSync:
                ...
                if success:
                    ...
                    if response:
                        if response.get("status") == "queued":
                            self.log.info(f"   Server queued job: {response.get('job_id', 'N/A')[:8]}...")
                        elif response.get("deferred"):
                            self.log.warning(f"   ⏸️  Server deferred {response['deferred']} events")
                else:
                    # ❌ FAILURE
    ...
    def _send_to_server(self, job: Dict) -> tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            if response.status_code in (200, 202):
                ...
                self.log.warning(f"⚠️  Duplicate detected (sync_id={job['sync_id'][:8]}...) - skipping")
                return True, None
    ...
    def _handle_failure(self, job: Dict):
        ...
        self.log.warning(f"⏸️  Retry in {backoff_time}s (attempt {job['attempt']}/{self.max_retry_attempts})")
