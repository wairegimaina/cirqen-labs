...
The view lives in ``calSchedules.views.tasks`` and imports the task and the
access helper by name, so they are patched there rather than on the package.
"""
...
class TriggerSmartReorganizeForwardingTests(TestCase):
    ...
    @patch("calSchedules.views.tasks.smart_reorganize_on_logic_change")
    @patch("calSchedules.views.tasks.get_user_access_context", return_value={"can_schedule": True})
    ...
    @patch("calSchedules.views.tasks.smart_reorganize_on_logic_change")
    @patch("calSchedules.views.tasks.get_user_access_context", return_value={"can_schedule": False})
