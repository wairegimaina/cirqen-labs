...
def export_equipment_to_excel(request):
    ...
    for item in equipments:
        ...
        ws.cell(row=row, column=7).value = item.asset_tag or ""
    ...
    headers = ["Description", "Manufacturer", "Model", "Serial Number", "Department", "Status",
               "Hospital Asset No."]
    # The template file only has the first six headers; add ours beside them.
    for col_num, title in enumerate(headers[6:], start=7):
        header_cell = ws.cell(row=2, column=col_num, value=title)
        source = ws.cell(row=2, column=6)
        if source.has_style:
            header_cell._style = copy(source._style)
