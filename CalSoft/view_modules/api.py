...
from CalSoft.models import (
    ...
    CalibrationSchedule, Equipment, CalibrationReading, SubParameter,
    Parameter, Standard, StandardParameter, SetValue
)
...
def api_calculate_uncertainty(request):
    ...
    try:
        ...
        stats = calculator.calculate_statistics([float(r) for r in readings])

        if stats:
            type_a = calculator.calculate_type_a_uncertainty(
                float(stats['std_dev']), stats['count'])
            type_b = calculator.calculate_type_b_uncertainty(float(resolution))
            combined = calculator.calculate_combined_uncertainty(
                float(type_a), float(type_b), float(reference_uncertainty) / float(coverage_factor))
            expanded = calculator.calculate_expanded_uncertainty(
                float(combined), float(coverage_factor))
